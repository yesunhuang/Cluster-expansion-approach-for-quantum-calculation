...
namespace ayaji {
	...
	public:
		...
		static OPTree SONormalize(LabelSeq seq);
		/// <summary>
		/// normalize a multi-operator, and return a new tree
		/// </summary>
		/// <returns>a new tree storing the normalized labelseq</returns>
		static OPTree MONormalize(LabelSeq seq);
		...
		friend void deb_printTree_(pNode& node, LabelSeq& seqBuf, int cSize);
#endif // DEB
		...
		/// <returns></returns>
		int insert(LabelSeq nodeSeq, Complex coef);
		/// <summary>
		...
		pNode search(LabelSeq nodeSeq) const;
		/// <summary>
		...
		static OPTree BuildFromPTree(PTree& posTree, LabelSeq seq);

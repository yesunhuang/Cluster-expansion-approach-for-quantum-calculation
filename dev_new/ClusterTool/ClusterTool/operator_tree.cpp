...
namespace ayaji {
	OPTree OPTree::SONormalize(LabelSeq seq) {
		int len = seq.size();
		/* mark */
	...
	OPTree OPTree::MONormalize(LabelSeq seq) {
		/* 默认输入符合格式,即无0 */
		_SortMOArray(seq);
		int len = seq.size();
	...
	OPTree::OPTree()
		: root(nullptr), childSize(0), nodeCount(0)
	{

	}

	OPTree::OPTree(int childSize)
		: root(nullptr), childSize(childSize), nodeCount(0)
	{
	}
	...
	int OPTree::insert(LabelSeq nodeSeq) {
		return insert(nodeSeq, Complex(1, 0));
	}

	int OPTree::insert(LabelSeq nodeSeq, Complex coef) {
		...
		if (root == nullptr) {
			// create root
			root = Node::createNode(childSize, 0, Complex(0, 0));
		}
		...
		if (nodeSeq[0] == 0) {
			...
			if (!root->hasChild(0)) {
				// create the node
				root->children[0] = Node::createNode(childSize, 0, Complex(0, 0), root);
			}
	...
	int OPTree::remove(LabelSeq nodeSeq) {
		pNode nowNode = root;
		for (auto it = nodeSeq.cbegin(); it != nodeSeq.cend(); ++it) {
			Label nowLabel = *it;
			if (!nowNode->hasChild(nowLabel)) {
	...
	pNode OPTree::search(LabelSeq nodeSeq) const {
		pNode nowNode = root;
		for (auto it = nodeSeq.cbegin(); it != nodeSeq.cend(); ++it) {
			Label nowLabel = *it;
			if (!nowNode->hasChild(nowLabel)) {
	...
	int OPTree::setChildSize(int childSize) {
		...
		if (this->root == nullptr) {
			// build root
			this->root = Node::createNode(childSize, 0, Complex(), nullptr);
			return 0;
		}
	...
	OPTree OPTree::BuildFromPTree(PTree& posTree, LabelSeq seq) {
	...
	int OPTree::normalize() {
		...
		while (!stack.empty()) {
			...
			if (!p->hasChildren() && p->value.isZero() && p != this->root) {
				v.push_back(p);
			}
		}
